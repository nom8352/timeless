...
<?php
	...
	$pp_photostream_row = get_option('pp_photostream_row');
	if(empty($pp_photostream_row))
	{
		$pp_photostream_row = 2;
	}
	...
	if(!empty($pp_photostream) && $photography_homepage_style != 'fullscreen_video')
	{
		...
		if(!empty($photos_arr) && $photography_screen_class != 'split' && $photography_screen_class != 'split wide' && $photography_homepage_style != 'fullscreen' && $photography_homepage_style != 'flow')
		{
			...
			wp_enqueue_script("photography-script-footer-gridrotator", admin_url('admin-ajax.php')."?action=photography_script_gridrotator&grid=footer_photostream&rows=".$pp_photostream_row, false, THEMEVERSION, true);
?>
<br class="clear"/>
