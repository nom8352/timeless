...
jQuery(document).ready(function() {
	jQuery('form#<?php echo esc_js($contact_form_id); ?>').submit(function() {
		jQuery('#<?php echo esc_js($response_id); ?> ul li.error').remove();
		
		var hasError = false;
		jQuery('.required_field').each(function() {
			if(jQuery.trim(jQuery(this).val()) == '' && !jQuery(this).hasClass('gdpr')) {
				var labelText = jQuery(this).prev('label').text();
				jQuery('#<?php echo esc_js($response_id); ?> ul').append('<li class="error"><?php echo esc_html_e('Please enter', 'photography' ); ?> '+labelText+'</li>');
				hasError = true;
			} else if(jQuery(this).hasClass('gdpr') && !jQuery(this).is(':checked')) {
				var labelText = jQuery(this).next('label').text();
				jQuery('#<?php echo esc_js($response_id); ?> ul').append('<li class="error"><?php echo esc_html_e('Please check', 'photography' ); ?> '+labelText+'</li>');
				hasError = true;
			} else if(jQuery(this).hasClass('email')) {
				var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,13})?$/;
				if(!emailReg.test(jQuery.trim(jQuery(this).val()))) {
					var labelText = jQuery(this).prev('label').text();
					jQuery('#<?php echo esc_js($response_id); ?> ul').append('<li class="error"><?php echo esc_html_e('Please enter valid', 'photography' ); ?> '+labelText+'</li>');
					hasError = true;
				}
			}
		});
