...
<div id="ppb_template">
	...
	<div class="ppb_tab">
		<ul>
	    	<li><a href="#tabs-my-templates"><?php esc_html_e('My Templates', 'photography' ); ?></a></li>
	    	<li><a href="#tabs-predefined-templates"><?php esc_html_e('Predefined Templates', 'photography' ); ?></a></li>
	    </ul>

		<!-- Display my templates options -->
		<div id="tabs-my-templates">
			<div id="template_save_form">
				<input type="text" id="ppb_new_template_name" name="ppb_new_template_name" value="" placeholder="<?php esc_html_e('Template Name', 'photography' ); ?>"/>
				<a id="ppb_do_save_template_button" class="button button-primary" href="<?php echo esc_url(admin_url('admin-ajax.php?action=photography_ppb_set_template&page_id='.$post->ID)); ?>"><?php esc_html_e('Save', 'photography' ); ?></a>
				
				<br/>
				<div class="pp_widget_description"><?php esc_html_e('Template Name must have at least 3 characters', 'photography' ); ?></div>
			</div>
			
			<!-- Display demo templates -->
			<div class="template_sub_title_bar">
				<h3><?php esc_html_e('Saved Templates', 'photography' ); ?></h3>
				<a href="javascript:;" class="inline_help tooltipster" title="<?php esc_html_e('Predefined templates are built-in theme using demo pages', 'photography' ); ?>"><span class="dashicons dashicons-warning"></span></a>
			</div>
			...
			?>
					<li id="ppb_my_page_<?php echo esc_attr($key); ?>" data-module="<?php echo esc_attr($key); ?>" data-title="<?php echo esc_attr($my_current_template); ?>" data-key="<?php echo esc_attr($key); ?>">
						...
						<a class="delete_link tooltipster" href="<?php echo esc_url(admin_url('admin-ajax.php?action=photography_ppb_remove_template&template_id='.$key)); ?>" title="<?php echo esc_html_e('Remove', 'photography' ); ?>"><span class="dashicons dashicons-no-alt"></span></a>
					</li>
			<?php		
			    	} //End foreach
			    }
			...
			?>
				<li id="ppb_demo_page_<?php echo esc_attr($key); ?>" data-module="<?php echo esc_attr($key); ?>" data-title="<?php echo esc_attr($ppb_demo_page['title']); ?>" data-type="demo_page" data-file="<?php echo esc_attr($ppb_demo_page['file']); ?>" data-key="<?php echo esc_attr($key); ?>">
			    	...
			    	<?php
					    ...
					    {
					?>
					<a class="preview_link tooltipster" href="<?php echo esc_url($ppb_demo_page['url']); ?>" target="_blank" title="<?php echo esc_html_e('Preview', 'photography' ); ?> <?php echo esc_html($ppb_demo_page['title']); ?>"><span class="dashicons dashicons-admin-site"></span></a></a>
					<?php
					    }
					?>
				</li>
