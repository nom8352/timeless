...
<div class="newsletter_form2_com_news">	
	<section class="row space newsletter_form2_c_get">
			<h2 class="newsletter_form2_section-heading wow flipInX"><?php echo esc_html($wl_nls_options['subscriber_form_title']);?></h2>			
			<?php if ($wl_nls_options['subscriber_form_icon'] == null) { ?><span class="newsletter_form2_section-icon wow rubberBand"> <span class="<?php echo esc_attr($wl_nls_options['subscriber_form_icon']);?> newsletter_form2_icon"></span> </span>
			<?php } else { ?>
			<span class="newsletter_form2_section-icon wow rubberBand">..........   <span class="<?php echo esc_attr($wl_nls_options['subscriber_form_icon']);?> newsletter_form2_icon"></span>   ..........</span>
			<?php } ?>
			<h3 class="newsletter_form2_section-sub_heading" data-sr="enter top"><?php echo esc_html($wl_nls_options['subscriber_form_sub_title']);?></h3>
	...
	<section class="row space newsletter_form2_c_get_detail">
		<?php if ($wl_nls_options['subscribe_select']=='wp_mail'){ ?>
				<script>
					function validateForm21_<?php echo esc_attr( $r_code);?>() {
				...
				<form method="post" action="" onsubmit="if(document.getElementById('terms').checked) { return true; } else { 	return true; }" class="subscriber-form" name="subscriber-form_<?php echo esc_attr($r_code);?>">
					...
					<div class="form-group" data-sr="enter bottom over 1s and move 110px wait 0.5s">	
						...
						<div class="col-md-12 form-group">
							<input type="text" name="f_name"  class="form-control wow slideInUp" placeholder="<?php if ($wl_nls_options['sub_form_button_f_name']){ echo esc_attr($wl_nls_options['sub_form_button_f_name']);} else {echo "Enter First Name";}?>" required='required'>
						</div>
						<div class="col-md-12 form-group">
							<input type="text" name="l_name"  class="form-control wow slideInUp " placeholder="<?php if ($wl_nls_options['sub_form_button_l_name']){ echo esc_attr($wl_nls_options['sub_form_button_l_name']);}else {echo "Enter Last Name";}?>" required='required'>
						</div>
						<?php } ?>
						<div class="col-md-12 form-group">
							<input type="email" name="subscribe_email" id="edmm-sub-email2" class="form-control wow slideInUp subscribe-input-layout1 s2email" placeholder="<?php echo esc_attr($wl_nls_options['sub_form_subscribe_title']);?>">						
							<span class="sub_error_msg_<?php echo esc_attr($r_code);?>" style="display:none; color:red;"><span style="color:red; font-size: 14px;"></br><?php esc_html_e('*','NLS_TEXT_DOMAIN' );?></span><?php esc_html_e("Invalid email address.","NLS_TEXT_DOMAIN"); ?></span>
						</div>
						...
						<div class="col-md-12">					
							<button name="submit_subscriber" class="btn wow slideInUp subscriber_submit sub-submit" type="submit"><?php echo esc_html($wl_nls_options['sub_form_button_text']);?></button>
						</div>
					</div>
				</form>
			...
			<script>
				function validateForm22_<?php echo esc_attr($r_code);?>() {
					var x = document.forms["php_subscriber-form_<?php echo esc_attr($r_code);?>"]["subscribe_email"].value;
					var atpos = x.indexOf("@");
					var dotpos = x.lastIndexOf(".");
					var error_msg = ".sub1_error_msg_<?php echo esc_attr($r_code);?>";
					if (atpos<1 || dotpos<atpos+2 || dotpos+2>=x.length) {
						jQuery(error_msg).show();
						jQuery(error_msg).fadeOut(3000);
						return false;
		...
		<div id="error_email2" class="validation_error error_email"></div>			
			<form method="post" action="" onsubmit="if(document.getElementById('terms').checked) { return true; } else { return true; }" class="php_subscriber-form" name="php_subscriber-form_<?php echo esc_attr($r_code);?>">
				...
				<div class="form-group" data-sr="enter bottom over 1s and move 110px wait 0.5s">					
					...
					<div class="col-md-12 form-group">
						<input type="email" name="subscribe_email" id="edmm-sub-email2" class="form-control wow slideInUp subscribe-input-layout1 s2email" placeholder="<?php echo esc_attr($wl_nls_options['sub_form_subscribe_title']);?>">
						<span class="sub1_error_msg_<?php echo esc_attr($r_code);?>" style="display:none; color:red;"><span style="color:red; font-size: 14px;"></br><?php esc_html_e('*','NLS_TEXT_DOMAIN' );?></span><?php esc_html_e("Invalid email address.","NLS_TEXT_DOMAIN"); ?></span>
					</div>
					<?php if($wl_nls_options['wl_gdpr_select']=='yes') { ?>
						<input type="checkbox" name="terms" id="terms" value="yes" > <?php esc_html_e(' I Agree with GDPR compliant Terms & Coditions','NLS_TEXT_DOMAIN' );?> 
						<?php } ?>	
					<div class="col-md-12">					
						<button name="php_submit_subscriber" class="btn wow slideInUp subscriber_submit sub-submit" type="submit"><?php echo esc_html($wl_nls_options['sub_form_button_text']);?></button>
					</div>	
				</div>
			</form>
			...
			<?php if($wl_nls_options['social_icons_onoff']=='on'){	?>
					<ul class="newsletter_form2_social animated fadeInDownBig">
						<?php for($i=1; $i<=$wl_nls_options['total_Social_links']; $i++){
							...
							?>
							<li class="newsletter_form2_social <?php echo esc_attr($social_class); ?>"><a target="<?php if($wl_nls_options['social_link_tab_'.$i]=='on') echo '_blank'; ?>" href="<?php echo esc_url($wl_nls_options['social_link_'.$i]); ?> <?php echo esc_attr($wl_nls_options['social_link_'.$i]); ?>"><i style="color:<?php echo esc_attr($social_color);?>!important;"  class="<?php echo esc_attr($wl_nls_options['social_icon_'.$i]); ?> newsletter_form2_icon"></i></a></li>
								<?php }
							} ?>
					</ul>
				<?php } ?>
