...
?>
	...
	<link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=<?php echo esc_attr($wl_nls_options['theme_font_family']); ?>">	
...
<div class="newsletter_form1_social-icons">
	<ul class="newsletter_form1_social animated fadeInDownBig">
		<?php for($i=1; $i<=$wl_nls_options['total_Social_links']; $i++){
			...
			?>
		<li><a target="<?php if($wl_nls_options['social_link_tab_'.$i]=='on') echo '_blank'; ?>" href="<?php echo esc_url($wl_nls_options['social_link_'.$i]); ?>"><i style="color:<?php echo esc_html($social_color);?>!important;" class="<?php echo esc_attr($wl_nls_options['social_icon_'.$i]); ?> newsletter_form1_icon"></i></a></li>
		<?php }
			} ?>
	</ul>		
</div>
<?php } ?>
	<h2 class="newsletter_form1_section-heading" data-sr="enter top"><?php echo esc_html($wl_nls_options['subscriber_form_title']);?></h2>
	<h4 class="newsletter_form1_section-sub_heading" data-sr="enter top"><?php echo esc_html($wl_nls_options['subscriber_form_sub_title']);?></h4>		
	<?php if ($wl_nls_options['subscriber_form_icon'] == null) { ?><span class="newsletter_form1_section-icon"><span class="<?php echo esc_attr($wl_nls_options['subscriber_form_icon']);?>  icon"></span></span>
	<?php } else { ?>
	<span class="newsletter_form1_section-icon"> .......... <span class="<?php echo esc_attr($wl_nls_options['subscriber_form_icon']);?>  newsletter_form1_icon"></span> .......... </span>
	<?php } ?>
	<p class="newsletter_form1_section-description"><?php echo esc_html($wl_nls_options['subscriber_form_message']);?> </p>
	
	<?php if($wl_nls_options['subscribe_select']=='wp_mail') { ?>
	<script>
		function validateForm11_<?php echo esc_attr($r_code);?>(){
			var x = document.forms["subscriber-form_<?php echo esc_attr($r_code);?>"]["subscribe_email"].value;
			var atpos = x.indexOf("@");
			var dotpos = x.lastIndexOf(".");
			var error_msg = ".sub_error_msg_<?php echo esc_attr($r_code);?>";
	...
	<form method="post" action="" onsubmit="if(document.getElementById('terms').checked) { return true; } else { return true; }" class="subscriber-form" name="subscriber-form_<?php echo esc_attr($r_code);?>">
		...
		<div class="form-group" data-sr="enter bottom over 1s and move 110px wait 0.5s">
			...
			<input type="text" name="f_name" id="f_sub_name"  class="form-control subscribe-input-layouts" placeholder="<?php if ($wl_nls_options['sub_form_button_f_name']){ echo esc_attr($wl_nls_options['sub_form_button_f_name']);} else {echo "First Name";}?>" required='required'>
			<input type="text" name="l_name" id="l_sub_name"  class="form-control subscribe-input-layouts" placeholder="<?php if ($wl_nls_options['sub_form_button_l_name']){ echo esc_attr($wl_nls_options['sub_form_button_l_name']);}else {echo "Last Name";}?>" required='required'>
			<?php } ?>				
			<input type="email" name="subscribe_email" id="edmm-sub-email1"  class="form-control subscribe-input-layout2" placeholder="<?php if ($wl_nls_options['sub_form_subscribe_title']){ echo esc_attr($wl_nls_options['sub_form_subscribe_title']);}else { echo "Email";}?>">
			<!--<input type="hidden" name="email_type" id="edmm-sub-email1"  class="form-control subscribe-input-layout2" value="<?php //echo "wp_mail" ?>"> -->
			<span class="sub_error_msg_<?php echo esc_attr($r_code);?>" style="display:none; color: red; width: 100%; float: left; margin-bottom: 10px; font-size: 14px;"><span style="color:red; font-size: 14px;"><?php esc_html_e('*','NLS_TEXT_DOMAIN' );?></span><?php esc_html_e("Invalid email address.","NLS_TEXT_DOMAIN"); ?></span>
			<input type="hidden" name="subscribe_type2" id="subscribe_type2"  class="form-control subscribe-input-layout2" value="<?php echo "subscribe-message_".esc_attr($r_code);?>">
			<br>
			...
			<?php if ($wl_nls_options['sub_form_button_text']){ echo esc_html($wl_nls_options['sub_form_button_text']);}else { echo "Subscribe";}?></button>
			</div>
		</div>
	</form>
...
jQuery('.newsletter_form1_subscribe-message1 .main_div').show();
	...
	<script>		
		function validateForm12_<?php echo esc_attr($r_code);?>() {
			var x = document.forms["php_subscriber-form_<?php echo esc_attr($r_code);?>"]["subscribe_email"].value;
			var atpos = x.indexOf("@");
			var dotpos = x.lastIndexOf(".");
			var error_msg = ".sub_error_msg_<?php echo esc_attr($r_code);?>";
			if (atpos<1 || dotpos<atpos+2 || dotpos+2>=x.length) {
	...
	<div id="error_email2" class="validation_error error_email"></div>	
	<form method="post" action="" onsubmit="if(document.getElementById('terms').checked) { return true; } else { return true; }" class="php_subscriber-form" name="php_subscriber-form_<?php echo esc_attr($r_code);?>">
		<?php $nonce = wp_create_nonce( 'php_subscriber_nonce_field' ); ?>
			...
			<div class="form-group" data-sr="enter bottom over 1s and move 110px wait 0.5s">					
				...
				<input type="text" name="l_name" id="l_sub_name"  class="form-control subscribe-input-layouts" placeholder="<?php if ($wl_nls_options['sub_form_button_l_name']){ echo esc_attr($wl_nls_options['sub_form_button_l_name']);}else {echo "Last Name";}?>" required='required'>	
				<?php } ?>
				<input type="email" name="subscribe_email" id="edmm-sub-email2"  class="form-control subscribe-input-layout1 s2email" placeholder="<?php echo esc_attr($wl_nls_options['sub_form_subscribe_title']);?>">
				<span class="sub_error_msg_<?php echo esc_attr($r_code);?>" style="display:none; color:red; width:100%; float:left; margin-bottom:10px; font-size:14px;"><span style="color:red; font-size: 14px;"><?php esc_html_e('*','NLS_TEXT_DOMAIN' );?></span><?php esc_html_e('Invalid email address.','NLS_TEXT_DOMAIN'); ?></span>
				<input type="hidden" name="subscribe_type2" id="subscribe_type2"  class="form-control subscribe-input-layout2" value="<?php echo "subscribe-message_".esc_html($r_code);?>">
